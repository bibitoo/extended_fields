...
module ExtendedCustomFieldPatch
    ...
    module AliasPossibleValuesOptionsMethod
	   ...
	   when 'customproject'
                if obj.is_a?(User)
                    customprojects = Customproject.visible(obj).all
                else
                    customprojects = Customproject.visible.all
                end
                customprojects.collect{ |customproject| [ customproject.sname, customproject.id.to_s ] }
            else
    ...
    module OverridePossibleValuesMethod
        ...
        def possible_values(dummy = nil)
            ...
            when 'customproject'
                Customproject.visible.all.collect{ |customproject| [ customproject.sname, customproject.id.to_s ] }
            else
    ...
    module AliasCastValueMethod
        ...
        def cast_value_with_extended(value)
            ...
            when 'customproject'
                unless value.blank?
                    Customproject.find_by_id(value)
                else
                    nil
                end
            else
